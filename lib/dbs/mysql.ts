...
// get all bundle categories form my sql database
// export async function getCategories() {
//     const results = await query('SELECT * FROM categories');
//     return results;
// }

export async function getBundleById(categoryId: number) {
    const results = await query('SELECT * FROM sub_categories WHERE category_id = ?', categoryId);
    return results;
}
